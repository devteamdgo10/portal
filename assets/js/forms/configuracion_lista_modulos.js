    ...
    success: function(data){
          ...
          if (data.Respuesta.length > 0){
            for (row in data.Respuesta){
              opciones = `<a href='configuracion_edita_modulo.html?idcf=${data.Respuesta[row].Id_Modulo}' title='Editar' class='fa-stack fa-lg'><i class='fa fa-edit fa-stack-1x'></i></a>`;
                            //<a title='Eliminar' href='#' onclick='EliminarEmpresa(${data.Respuesta[row].Id_Empresa},\"${data.Respuesta[row].Empresa.trim()}\")' class='fa-stack fa-lg'><i class='fa fa-trash-o fa-stack-1x'></i></a>`;
              Estatus = data.Respuesta[row].Estatus == 1 ? 'Activo':"Inactivo";
              listado_sistemas.push([data.Respuesta[row].Id_Modulo,data.Respuesta[row].Modulo.trim(), Estatus, opciones]);
            }
          }
...
$(document).ready(function() {
	//$("#tree_"+_menu).addClass('active');
  //$("#"+link).addClass('active');

  //console.log("path=> ", pathWs)
  //console.log("Listo para mostrar tabla ")
  respValida = validar()
  if (respValida){
    lista()
  }
